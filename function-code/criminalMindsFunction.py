...
def function13():
    ...
    #Imagem póster
    img = PhotoImage(file="criminal-minds-poster.gif")
    poster_space.create_image(253, 375, image=img)

    #Campo de título
    titulo = Label(page, text="Criminal Minds (2005)", fg="#5E239D", font=("Calisto MT", "16", "bold"))
    ...
    cat2 = Label(page, text="Ação, Mistério", font=("Calisto MT", "10"))
    ...
    clas2 = Label(page, text="8.1", font=("Calisto MT", "10"))
    ...
    cast2 = Label(page, text="Mantegna, Matthew Gray Gubler", font=("Calisto MT", "10"))
    cast2.place(x=400, y=100)

    #Campo de descrição
    desc = Text(page, height=6, width=52, font=("Calisto MT", "10", "bold"))
    desc.place(x=340, y=125)
    desc.insert(1.0, "A história acompanha um grupo de elite Unidade de Análise Comportamental do FBI, que analisa perfis dos assassinos em série mais perigosos do país e crimes hediondos individuais, em um esforço para antecipar seus próximos movimentos antes de atacar novamente.")
    ...
    videoplayer.load(r"criminal-minds-trailer.mp4")
    ...
    def remover():
        ...
        coment.set("")
        f = open("com_criminal.txt", "w", encoding="utf-8")
    ...
    def num_likes():
        f=open("like_criminal.txt", "r", encoding="utf-8")
        ficheiro=f.readlines()
        f.close()
        count=0
        for linha in ficheiro:
