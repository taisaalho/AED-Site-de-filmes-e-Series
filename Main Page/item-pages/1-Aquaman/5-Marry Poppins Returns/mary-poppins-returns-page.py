...
window.title("Mary Poppins Returns")
...
#Imagem póster
img = PhotoImage(file="mary-poppins-returns-poster.gif")
poster_space.create_image(253, 375, image=img)

#Campo de título
titulo = Label(window, text="Mary Poppins Returns (2018)", fg="#5E239D", font=("Calisto MT", "16", "bold"))
...
cat2 = Label(window, text="Fantasia, Comédia", font=("Calisto MT", "10"))
...
clas2 = Label(window, text="6.7", font=("Calisto MT", "10"))
...
cast2 = Label(window, text="Emily Blunt, Lin Manuel Miranda", font=("Calisto MT", "10"))
cast2.place(x=400, y=100)

#Campo de descrição
desc = Text(window, height=4, width=50, font=("Calisto MT", "10", "bold"))
desc.place(x=340, y=120)
desc.insert(1.0, "Em uma Londres abalada pela Grande Depressão, Mary Poppins desce dos céus novamente com seu fiel amigo Jack para ajudar Michael e Jane Banks, agora adultos trabalhadores que sofreram uma grande perda. As crianças Annabel, Georgie e John vivem com os pais na mesma casa de há 24 anos atrás e precisam da enigmática Mary Poppins e o otimista acendedor de candeeiros para trazer alegria e magia de volta para suas vidas.")
...
videoplayer.load(r"mary-poppins-returns-trailer.mp4")
...
def salvar_alteracoes():
    ...
    if val.get() == 1 and val1.get() == 0:
        ...
        frase = "Mary Poppins Returns (2018)" + "\n"
        f.write(frase)
        f.close
...
def remover():
    ...
    coment.set("")
    f = open("com_marypoppins.txt", "w", encoding="utf-8")
...
def num_likes():
    f=open("like_marypoppins.txt", "r", encoding="utf-8")
    ficheiro=f.readlines()
    f.close()
    count=0
    for linha in ficheiro:
