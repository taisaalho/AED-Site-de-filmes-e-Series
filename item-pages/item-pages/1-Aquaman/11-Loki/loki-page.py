...
#Imagem póster
img = PhotoImage(file="loki-poster.gif")
poster_space.create_image(253, 375, image=img)

#Campo de título
titulo = Label(window, text="Loki (2021)", fg="#5E239D", font=("Calisto MT", "16", "bold"))
...
cat2 = Label(window, text="Ação, Fantasia", font=("Calisto MT", "10"))
...
clas2 = Label(window, text="8.3", font=("Calisto MT", "10"))
...
cast2 = Label(window, text="Tom Hiddleston, Owen Wilson", font=("Calisto MT", "10"))
cast2.place(x=400, y=100)

#Campo de descrição
desc = Text(window, height=4, width=50, font=("Calisto MT", "10", "bold"))
desc.place(x=340, y=120)
desc.insert(1.0, "Depois de roubar o Tesseract durante os eventos de Vingadores: Ultimato, uma versão alternativa de Loki é trazida para a misteriosa Autoridade de Variância Temporal (AVT), uma organização burocrática que existe fora do tempo e do espaço, e monitora a linha do tempo.")
...
videoplayer.load(r"loki-trailer.mp4")
...
def remover():
    ...
    coment.set("")
    f = open("com_loki.txt", "w", encoding="utf-8")
...
def num_likes():
    f=open("like_loki.txt", "r", encoding="utf-8")
    ficheiro=f.readlines()
    f.close()
    count=0
    for linha in ficheiro:
