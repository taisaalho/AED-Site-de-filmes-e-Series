...
window.title("Over the Garden Wall")
...
#Imagem póster
img = PhotoImage(file="over-the-garden-wall-poster.gif")
poster_space.create_image(253, 375, image=img)

#Campo de título
titulo = Label(window, text="Over the Garden Wall (2014)", fg="#5E239D", font=("Calisto MT", "16", "bold"))
...
clas2 = Label(window, text="8.8", font=("Calisto MT", "10"))
...
cast2 = Label(window, text="Elijah Wood, Collin Dean", font=("Calisto MT", "10"))
cast2.place(x=400, y=100)

#Campo de descrição
desc = Text(window, height=4, width=50, font=("Calisto MT", "10", "bold"))
desc.place(x=340, y=120)
desc.insert(1.0, "A série conta o caminho de dois irmãos que viajam através de uma estranha floresta para encontrar o caminho de volta para casa. ... A série apresenta Elijah Wood e Collin Dean como os protagonistas Wirt e Greg, e Melanie Lynskey como a pássara azul Beatrice.")
...
videoplayer.load(r"over-the-garden-wall-trailer.mp4")
...
def salvar_alteracoes():
    ...
    if val.get() == 1 and val1.get() == 0:
        ...
        frase = "Over the Garden Wall (2014)" + "\n"
        f.write(frase)
        f.close
...
def remover():
    ...
    coment.set("")
    f = open("com_gardenwall.txt", "w", encoding="utf-8")
...
def num_likes():
    f=open("like_gardenwall.txt", "r", encoding="utf-8")
    ficheiro=f.readlines()
    f.close()
    count=0
    for linha in ficheiro:
