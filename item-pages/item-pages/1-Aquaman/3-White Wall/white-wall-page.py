...
#Imagem póster
img = PhotoImage(file="white-wall-poster.gif")
poster_space.create_image(253, 375, image=img)

#Campo de título
titulo = Label(window, text="White Wall (2010)", fg="#5E239D", font=("Calisto MT", "16", "bold"))
...
cat2 = Label(window, text="Ação, Mistério", font=("Calisto MT", "10"))
...
clas2 = Label(window, text="4.2", font=("Calisto MT", "10"))
...
cast2 = Label(window, text="James Boss, Gary Kohn", font=("Calisto MT", "10"))
cast2.place(x=400, y=100)

#Campo de descrição
desc = Text(window, height=4, width=50, font=("Calisto MT", "10", "bold"))
desc.place(x=340, y=120)
desc.insert(1.0, "Situado numa terra devastada, cercada por um grande muro branco, o último sobrevivente de um brutal campo de internamento de crianças assume relutantemente o último trabalho de caça à recompensa para proteger a ideológica vida que ele reconstruiu.")
...
videoplayer.load(r"white-wall-trailer.mp4")
...
def remover():
    ...
    coment.set("")
    f = open("com_whitewall.txt", "w", encoding="utf-8")
...
def num_likes():
    f=open("like_whitewall.txt", "r", encoding="utf-8")
    ficheiro=f.readlines()
    f.close()
    count=0
    for linha in ficheiro:
