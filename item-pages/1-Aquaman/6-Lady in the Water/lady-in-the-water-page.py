...
window.title("Lady in the Water")
...
#Imagem póster
img = PhotoImage(file="lady-in-the-water-poster.gif")
poster_space.create_image(253, 375, image=img)

#Campo de título
titulo = Label(window, text="Lady in the Water (2006)", fg="#5E239D", font=("Calisto MT", "16", "bold"))
...
cat2 = Label(window, text="Fantasia, Mistério", font=("Calisto MT", "10"))
...
clas2 = Label(window, text="5.5", font=("Calisto MT", "10"))
...
cast2 = Label(window, text="Bryce Dallas Howard, Paul Giamatti", font=("Calisto MT", "10"))
cast2.place(x=400, y=100)

#Campo de descrição
desc = Text(window, height=4, width=50, font=("Calisto MT", "10", "bold"))
desc.place(x=340, y=120)
desc.insert(1.0, "Quando Cleveland salva uma jovem enigmática do perigo, ele não se dá conta de que ela é uma personagem de histórias infantis que está a temtar retornar ao lugar de onde veio. à medida que o tempo vai passando, Cleveland apaixona-se cada vez mais pela rapariga e acaba percebendo que ele e os inquilinos do prédio também são personagens do conto e devem fazer de tudo para a proteger de criaturas malignas que tentam impedir que ela retorne ao seu mundo.")
...
videoplayer.load(r"lady-in-the-water-trailer.mp4")
...
def remover():
    ...
    coment.set("")
    f = open("com_ladywater.txt", "w", encoding="utf-8")
...
def num_likes():
    f=open("like_ladywater.txt", "r", encoding="utf-8")
    ficheiro=f.readlines()
    f.close()
    count=0
    for linha in ficheiro:
